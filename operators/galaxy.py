...
class ALBPY_OT_CreateGalaxy(Operator):
    ...
    bl_description = "Create a galaxy object with realistic morphological and photometric properties"
    ...
    galaxy_type: EnumProperty(
        ...
        items=[
            ...
            ('SBb', 'SBb - Barred Spiral (moderate)', 'Barred spiral with moderate arms'),
    ...
    def execute(self, context: bpy.types.Context) -> Set[str]:
        ...
        try:
            ...
            from ..utilities.galaxy_utilities import get_galaxy_properties, get_galaxy_morphology_params
            ...
            if self.add_bulge_disk and not morph_params["is_irregular"]:
                ...
                if morph_params["bulge_fraction"] > 0.05:  # Significant bulge
                    bulge_obj = self._create_galaxy_bulge(context, galaxy_props, morph_params)
                    galaxy_objects.append(bulge_obj)
                
                if morph_params["disk_fraction"] > 0.05:  # Significant disk
                    disk_obj = self._create_galaxy_disk(context, galaxy_props, morph_params)
                    galaxy_objects.append(disk_obj)
            else:
                # Create single galaxy object
                galaxy_obj = self._create_single_galaxy(context, galaxy_props, morph_params)
            ...
            self.report({'INFO'}, f"Created {self.galaxy_type} galaxy with {len(galaxy_objects)} components")
            return {'FINISHED'}
    ...
    def _create_galaxy_bulge(self, context: bpy.types.Context, 
                           galaxy_props: dict, morph_params: dict) -> bpy.types.Object:
        ...
        bulge_ellipticity = min(morph_params["ellipticity"], 0.7)  # Bulges are less flat
    ...
    def _create_galaxy_disk(self, context: bpy.types.Context,
                          galaxy_props: dict, morph_params: dict) -> bpy.types.Object:
    ...
    def _create_single_galaxy(self, context: bpy.types.Context,
                            galaxy_props: dict, morph_params: dict) -> bpy.types.Object:
        """Create single galaxy object."""
    ...
    def _apply_galaxy_material(self, obj: bpy.types.Object, 
                             galaxy_props: dict, morph_params: dict) -> None:
...
class ALBPY_OT_CreateGalaxyCluster(Operator):
        ...
        try:
            ...
            if self.central_galaxy:
                ...
                central_galaxy = self._create_cluster_galaxy(
                    context, 'E0', position=(0, 0, 0), 
                    stellar_mass=50.0, effective_radius=20.0
                )
            ...
            for i in range(start_index, self.galaxy_count):
                ...
                r = np.random.exponential(self.cluster_radius / 3)  # Exponential radial profile
                ...
                stellar_mass = np.random.lognormal(np.log(5.0), 0.7)  # Log-normal mass function
                ...
                galaxy = self._create_cluster_galaxy(
                    context, galaxy_type, position=(x, y, z),
                    stellar_mass=stellar_mass, effective_radius=effective_radius
                )
                cluster_galaxies.append(galaxy)
            
            # Group galaxies in collection
            cluster_collection = bpy.data.collections.new(f"Galaxy_Cluster_z{self.redshift:.2f}")
            ...
            self.report({'INFO'}, f"Created galaxy cluster with {len(cluster_galaxies)} galaxies")
            return {'FINISHED'}
    ...
    def _create_cluster_galaxy(self, context: bpy.types.Context, galaxy_type: str,
                             position: tuple, stellar_mass: float, 
                             effective_radius: float) -> bpy.types.Object:
