...
class ALBPY_OT_CreateStar(Operator):
    ...
    bl_description = "Create a star object with realistic stellar properties and materials"
    ...
    def execute(self, context: bpy.types.Context) -> Set[str]:
        ...
        try:
            ...
            self.report({'INFO'}, f"Created {self.spectral_class}-class star: {star_obj.name}")
            return {'FINISHED'}
    ...
    def _create_star_mesh(self, context: bpy.types.Context) -> bpy.types.Object:
        ...
        # Create object
        star_obj = bpy.data.objects.new(f"Star_{self.spectral_class}_{self.temperature:.0f}K", mesh)
            ...
            if stellar_mat.node_tree:
                for node in stellar_mat.node_tree.nodes:
                    if node.type == "GROUP" and "ALBPY_Stellar" in str(node.node_tree):
                        ...
                        node.inputs["Corona Intensity"].default_value = self.corona_intensity

    def _create_stellar_lighting(self, context: bpy.types.Context, star_obj: bpy.types.Object) -> None:
